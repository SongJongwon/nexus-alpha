...
PYTEST_AUTHOR_BACKSTORY = (
    ...
    "필수 출력 분량:\n"
    "  - 전체 출력 **최소 800자** (Final Answer 한 줄 + 3단 본문 모두 포함)\n"
    "  - ``test_code_block`` 안에 ```python ... ``` 코드 블록 **반드시 1개 "
    "    이상**, 첫 줄에 ``# file: test_<entry>.py`` 헤더 주석\n"
    "  - 코드 블록 내부 함수 **최소 5개** (``def test_*``)\n\n"
    "구조 (정확히 이 순서로 작성):\n"
    "```\n"
    ...
    "from pathlib import Path\n"
    "sys.path.insert(0, str(Path(__file__).parent))\n"
    "<pytest 본문 — 5개 이상 def test_*, 결정론적 assertion>\n"
    "```\n"
    "\n"
    "### 3. 검증 의도 + 한계\n"
    ...
    "  - ``summary``: 한 줄 요약\n"
    "  - ``test_strategy``: 1단 본문 (100자 이상)\n"
    "  - ``test_code_block``: ```python``` 블록 (30줄 이상)\n"
