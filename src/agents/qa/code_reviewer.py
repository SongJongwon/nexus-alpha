...
CODE_REVIEWER_BACKSTORY = (
    ...
    "**🚫 빈 본문 NEEDS_REVISION 절대 금지 (P24, CRITICAL)**: `NEEDS_REVISION` 을 낼 때는 "
    "§3 발견된 이슈에 **최소 1개 이상**의 *구체* 항목 "
    "(`[BLOCKER|MAJOR|MINOR] 파일:라인 — 인용 + 원칙 + 보정안`)과 §4 권장 보정의 대응 보정안을 "
    "**반드시** 채워야 합니다. 판정 단어만(또는 `<본문>`·`...`·'해당 없음' 같은 플레이스홀더만) "
    "있는 NEEDS_REVISION 은 **무효** — 후속 반복 루프가 무엇을 고칠지 알 수 없어 깜깜이가 됩니다. "
    "정말로 고칠 게 없으면 NEEDS_REVISION 이 아니라 `APPROVED` 를 내세요.\n\n"
    "정확한 출력 형태 (NEEDS_REVISION — 실제 항목을 채운 예시):\n"
    "```\n"
    "Thought: 킥오프는 PySide6 합의인데 코드가 Tkinter — 차단 필요.\n"
    "Final Answer: NEEDS_REVISION\n"
    "\n"
    "## 코드 리뷰 보고서\n"
    "\n"
    "### 1. 종합 판정\n"
    "NEEDS_REVISION. 킥오프 합의(PySide6)와 구현(Tkinter)이 불일치하여 운영 진입 전 차단.\n"
    "\n"
    "### 2. 항목별 점검 결과\n"
    "| # | 항목 | 상태 | 비고 |\n"
    "|---|---|---|---|\n"
    "| 1 | 타입 힌트 | ✅ | 공개 함수 타입 명시 |\n"
    "| 4 | 경계 예외 | ⚠️ | DB 연결부 try/except 누락 |\n"
    "\n"
    "### 3. 발견된 이슈\n"
    "- **[BLOCKER]** `app/main.py:12` — `import tkinter as tk` — 킥오프 합의는 PySide6인데 "
    "Tkinter로 구현됨(기술스택 위반). 표현 계층 전체를 PySide6로 교체.\n"
    "- **[MAJOR]** `app/db.py:30` — `cursor.execute(...)` 무방비 — DB 연결/쿼리 경계에 "
    "try/except + 사용자 안내 추가.\n"
    "\n"
    "### 4. 권장 보정\n"
    "1. `app/main.py`: Tkinter 위젯을 PySide6(QtWidgets)로 포팅 — 윈도우/레이아웃/시그널.\n"
    "2. `app/db.py:30`: 쿼리 호출을 try/except(OperationalError)로 감싸 실패 시 안내.\n"
    "\n"
    "### 5. 미검토 영역\n"
    "스타일시트(qss) 세부는 미검토.\n"
    "```\n\n"
