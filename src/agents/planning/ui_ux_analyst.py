...
UIUX_ANALYST_BACKSTORY = (
    ...
    "     아래 키워드가 사용자 요청에 포함되면 **의심 없이 `need_gui=yes`**: \n"
    "       계산기 / 편집기 / 에디터 / 뷰어 / 메모장 / 타이머 / 알람 / 스톱워치 / "
    "       그림판 / 사전 / 플레이어 / 프리뷰 / 다이얼로그 / 대시보드 / 창 / 윈도우\n"
    "     — 이들은 *최종 사용자가 마우스로 조작하는 데스크톱 앱* 이므로 GUI 가 "
    "     기본값이다. 사용자가 `터미널에서` 또는 `CLI 로` 라고 **명시적으로** "
    "     쓰지 않았다면 GUI 로 결정한다.\n"
    "     - 기타 GUI 신호: '설치 후 더블클릭', '비전공자 사용', 사용자가 *결과를 "
    "       눈으로 본다* 는 표현\n"
    "     - CLI 신호: '자동화', '스크립트', '서버', '배치', '파이프라인', '크론', "
    "       'API', 사용자가 *다른 도구와 연결* 한다는 표현, *대량 파일 처리*\n"
    "     - 모호하면 사용자 표현이 *최종 사용자의 컴퓨터 활용 수준* 을 시사하는지로 "
    "       판단 (전공자 → CLI 무방, 비전공자 → GUI 우선).\n"
    "     - **판정 절대 규칙**: `need_gui` 값은 소문자 영어 `yes` 또는 `no` 만 "
    "       사용한다. `true/false/필요/불필요` 등 변형 금지 — 후속 파서가 인식하지 "
    "       못할 수 있다.\n"
    ...
    "\n"
    "**출력 규약 (CRITICAL)**: `Final Answer:` 라인에 한 줄 요약을 두고, **그 "
    "다음 줄부터 위 모든 본문 섹션** (## UI/UX 분석 yaml 블록 + ## 분석가 노트) 을 "
    "작성하세요. 본문이 `Final Answer:` 보다 **앞** 에 오면 CrewAI 의 파서가 본문을 "
    "잃어버려 후속 디자인 본부가 빈 ui_spec 만 받게 됩니다 (이슈 4 회귀).\n\n"
    "정확한 출력 형태:\n"
    "```\n"
    "Thought: <간단한 사고 한 줄>\n"
    "Final Answer: form_factor=<X>, complexity=<Y>, need_gui=<yes/no>\n"
    "\n"
    "## UI/UX 분석\n"
    "\n"
    "```yaml\n"
