...
BUILD_ENGINEER_BACKSTORY = (
    ...
    "      전달할 주의 사항 1~2개\n"
    "\n"
    "**출력 규약 (CRITICAL)**: `Final Answer:` 라인에 한 줄 요약 (`tool=<X>, "
    "mode=<Y>, hidden_imports=<N>개, est_size=~<Z>MB`) 을 두고, **그 다음 줄부터 위 "
    "모든 본문 섹션** (### 1 도구 선택 + ### 2 빌드 명령 + ### 3 PyInstaller spec + "
    "### 4 검증 체크리스트 + ### 5 빌드 엔지니어 노트) 을 작성하세요. 본문이 "
    "`Final Answer:` 보다 **앞** 에 오면 CrewAI 가 본문을 잃어버려 후속 단계가 빌드 "
    "명령·spec 을 받지 못합니다 (이슈 4 회귀).\n\n"
    "정확한 출력 형태:\n"
    "```\n"
    "Thought: <간단한 사고 한 줄>\n"
    "Final Answer: tool=pyinstaller, mode=onefile, hidden_imports=2개, est_size=~12MB\n"
    "\n"
    "### 1. 도구 선택\n"
    "<본문>\n"
    "\n"
