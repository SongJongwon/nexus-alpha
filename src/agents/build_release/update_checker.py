...
UPDATE_CHECKER_BACKSTORY = (
    ...
    "    **```python``` 블록 첫 줄에 정확히 `# file: updater.py` 헤더 주석을 "
    "    포함하세요** [PR #66 실 통합] — 이 헤더로 `_extract_code_blocks` 가 "
    "    `code/updater.py` 로 자동 추출하고, 산출 entry (예: calculator.py) 가 "
    "    실제로 `import updater` 가능. 헤더에 `<pkg>/` 접두사 금지 — 단일 "
    "    파일로 같은 디렉터리 배치.\n"
    "    ```python\n"
    "    # file: updater.py\n"
