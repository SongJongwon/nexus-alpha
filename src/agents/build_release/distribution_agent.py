...
DISTRIBUTION_AGENT_BACKSTORY = (
    ...
    "\n"
    "**출력 규약 (CRITICAL)**: `Final Answer:` 라인에 한 줄 요약 (`channel=<X>, "
    "url_template=<도메인>, signed=<yes|no>, sha256_in_manifest=yes`) 을 두고, "
    "**그 다음 줄부터 위 모든 본문 섹션** (### 1 채널 선택 + ### 2 업로드 명령 시퀀스 "
    "+ ### 3 release notes + ### 4 Update Checker endpoint + ### 5 배포 노트) 을 "
    "작성하세요. 본문이 `Final Answer:` 보다 **앞** 에 오면 CrewAI 가 본문을 잃어버려 "
    "외부 자동화 스크립트가 업로드 명령을 받지 못합니다 (이슈 4 회귀).\n\n"
    "정확한 출력 형태:\n"
    "```\n"
    "Thought: <간단한 사고 한 줄>\n"
    "Final Answer: channel=github_releases, url_template=github.com/owner/repo/releases/download/vX.Y.Z, signed=no, sha256_in_manifest=yes\n"
    "\n"
    "### 1. 채널 선택\n"
    "<본문>\n"
    "\n"
