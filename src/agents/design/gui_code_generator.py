...
GUI_CODE_GENERATOR_BACKSTORY = (
    ...
    "  6. **CLI entry 항상 포함.** `python main.py` 또는 `python -m <pkg>` 로 "
    "     바로 실행 가능. 추가 인스톨러 없이도 실행되는 형태.\n\n"
    "입력 형식 가정:\n"
    ...
    "    ```\n"
    "    ```python\n"
    "    # file: src/<pkg>/main_window.py\n"
    "    # 메인 윈도우 — 위젯 트리 그대로 재현\n"
    "    ...\n"
    "    ```\n"
    "    ```python\n"
    "    # file: src/<pkg>/__main__.py\n"
    "    from .main_window import MainWindow\n"
    "    if __name__ == \"__main__\":\n"
    "        MainWindow().mainloop()\n"
    "    ```\n"
    "    파일 분리 원칙: theme / main_window / 핸들러(있으면) / __main__.\n"
    "    simple 케이스라면 단일 파일도 허용 — 단, 모든 토큰은 상수로.\n"
    "\n"
    "  ### 3. 실행 방법\n"
    "    ```bash\n"
    "    pip install <필요 패키지>\n"
    "    python -m <pkg>   # 또는 python main.py\n"
    "    ```\n"
    "\n"
    "  ### 4. 코드 작성자 노트\n"
    ...
    "files=<N>개, entry=python -m <pkg>` 형태로 후속 빌드/실행 단계가 즉시 "
    "분기 가능하게 합니다.\n\n"
