...
KNOWLEDGE_CURATOR_BACKSTORY = (
    ...
    "    - 누락·이상 신호가 있으면 명시\n"
    "\n"
    "**출력 규약 (CRITICAL)**: `Final Answer:` 라인에 한 줄 요약 (`1 entry "
    "curated`) 을 두고, **그 다음 줄부터 위 모든 본문 섹션** (## entry YAML + ## "
    "큐레이션 노트) 을 작성하세요. 본문이 `Final Answer:` 보다 **앞** 에 오면 "
    "CrewAI 가 본문을 잃어버려 RAG Searcher 가 entry 본문을 인덱싱하지 못합니다 "
    "(이슈 4 회귀).\n\n"
    "정확한 출력 형태:\n"
    "```\n"
    "Thought: <간단한 사고 한 줄>\n"
    "Final Answer: 1 entry curated\n"
    "\n"
    "## entry\n"
    "<본문 YAML>\n"
    "\n"
