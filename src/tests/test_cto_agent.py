...
# ---------------------------------------------------------------------------
def test_cto_agent_runs_through_crew_with_fake_provider() -> None:
    """FakeProvider 응답이 CrewAI 파서를 거쳐 AgentFinish로 정상 수렴하는지 검증한다."""
    cto = create_cto_agent(verbose=False)

    # FakeProvider가 factory 레벨에서 자동 주입되었는지 확인
    assert cto.llm.backend_provider.name == "fake"
    ...
    result = Crew(agents=[cto], tasks=[task], verbose=False).kickoff()
    output_text = getattr(result, "raw", None) or str(result)

    assert output_text.strip(), "CrewAI kickoff 결과가 비어 있으면 안 된다"
