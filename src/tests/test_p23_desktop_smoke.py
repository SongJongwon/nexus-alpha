...
class TestRunDesktopSmokeGate:
    def test_pass_when_runtime_alive(self, tmp_path: Path) -> None:
        ...
        r = run_desktop_smoke_gate(
            ...
            _runtime_test=lambda p, timeout_sec: _rt("PASS"),
            _enum_titles=lambda: [],
        )
    ...
    def test_fail_when_runtime_crash(self, tmp_path: Path) -> None:
        ...
        r = run_desktop_smoke_gate(
            exe, timeout_sec=0.2, settle_sec=0.0,
            _runtime_test=lambda p, timeout_sec: _rt("CRASH", exit_code=3, stderr="boom"),
            _enum_titles=lambda: [],
        )
    ...
    def test_skipped_when_exe_missing(self, tmp_path: Path) -> None:
        ...
        r = run_desktop_smoke_gate(
            missing, _runtime_test=lambda p, timeout_sec: _rt("PASS"), _enum_titles=lambda: []
        )
    ...
    def test_exception_is_graceful_skip(self, tmp_path: Path) -> None:
        ...
        def _boom(p, timeout_sec):
            raise RuntimeError("spawn explode")

        r = run_desktop_smoke_gate(
            exe, timeout_sec=0.2, settle_sec=0.0, _runtime_test=_boom, _enum_titles=lambda: []
        )
    ...
    def test_error_dialog_window_detected_while_alive(self, tmp_path: Path) -> None:
        """실행 중 *새로* 나타난 오류 다이얼로그 창 → FAIL (process 생존이라도)."""
        exe = _make_fake_exe(tmp_path)
        calls = {"n": 0}

        def _stateful_enum():
            calls["n"] += 1
            # 1회차(실행 전 스냅샷): 오류창 없음. 2회차+(실행 중): 오류창 등장.
            return [] if calls["n"] == 1 else ["치명적 오류 - DB 연결 실패"]

        def _alive(p, timeout_sec):
            import time as _t

            _t.sleep(0.2)  # 스캐너가 창을 잡을 시간
            return _rt("PASS")

        r = run_desktop_smoke_gate(
            exe, timeout_sec=0.4, settle_sec=0.0, _runtime_test=_alive, _enum_titles=_stateful_enum
        )
        assert r.verdict == "FAIL" and r.signal == "window"
